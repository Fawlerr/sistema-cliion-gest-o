...
export function Sidebar({ items, activePage, onChange, isOpen = false, onClose }) {
  return (
    <>
      <div
        className={`fixed inset-0 z-30 bg-slate-950/60 transition xl:hidden ${isOpen ? "opacity-100" : "pointer-events-none opacity-0"}`}
        onClick={onClose}
      />

      <aside
        className={`panel-border glass-panel fixed inset-y-4 left-4 z-40 flex w-[min(88vw,320px)] flex-col rounded-[32px] p-5 transition duration-300 xl:static xl:h-full xl:w-auto xl:translate-x-0 ${
            ...
            return (
              <button
                ...
                className={`w-full rounded-[22px] px-4 py-3 text-left transition duration-200 ${
                  isActive
                    ? "bg-[linear-gradient(135deg,rgba(20,184,166,0.28),rgba(56,189,248,0.18))] text-white shadow-[0_18px_48px_rgba(15,118,110,0.28)]"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-xs ${isActive ? "text-cyan-100/80" : "text-[color:var(--text-muted)]"}`}>
                      {item.description}
                    </p>
                  </div>
                </div>
